...
@~/.claude/buildsensei/references/questioning.md
@~/.claude/buildsensei/templates/project.md
@~/.claude/buildsensei/templates/config.json
...
1. **Abort if project exists:**
   ```bash
   [ -f .planning/PROJECT.md ] && echo "ERROR: Project already initialized. Use /sensei:progress" && exit 1
   ```
...
- options:
  - "Map codebase first" — Run /sensei:map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**
```
Run `/sensei:map-codebase` first, then return to `/sensei:new-project`
```
Exit command.
...
<step name="generate_context_files">

## Generate Initial Context Files

Generate CLAUDE.md skeleton and empty companion files for knowledge persistence.

**Reference:** @~/.claude/buildsensei/workflows/generate-context-files.md

1. **Create CLAUDE.md skeleton:**
   - Extract project name, description, core value from PROJECT.md
   - Extract tech stack from user answers (if provided)
   - Extract constraints as "Do Not" items
   - Use template from templates/context-files/claude-md.md
   - Mark sections that will be populated later with placeholders

2. **Create empty companion files:**
   ```bash
   # Create empty files for future population
   touch skills.md references.md
   ```

3. **Commit context files:**
   ```bash
   git add CLAUDE.md skills.md references.md
   git commit -m "docs(context): Initialize project context files

   - CLAUDE.md skeleton with project overview
   - Empty skills.md and references.md for future population

   Co-Authored-By: BuildSensei <noreply@buildsensei.dev>"
   ```

**CLAUDE.md skeleton content:**
```markdown
# CLAUDE.md - Project Context

<!--
  AUTO-GENERATED by BuildSensei
...
<!-- Will be populated as project builds -->
...
Present completion with next steps (see ~/.claude/buildsensei/references/continuation-format.md):
...
- Context: CLAUDE.md (skeleton - will grow with project)
- Skills: skills.md (empty - will populate as patterns emerge)
- References: references.md (empty - will populate as APIs encountered)
...
- [ ] CLAUDE.md skeleton created with project overview
- [ ] Empty skills.md and references.md created
