...
Example: `/sensei:insert-phase 72 Fix critical auth bug`
→ after = 72
...
if [ $# -lt 2 ]; then
  ...
  echo "Usage: /sensei:insert-phase <after> <description>"
  echo "Example: /sensei:insert-phase 72 Fix critical auth bug"
  exit 1
fi
```
...
2. Insert new phase heading with (INSERTED) marker:
   ...
   **Plans:** 0 plans
   ...
   Plans:
   - [ ] TBD (run /sensei:plan-phase {decimal_phase} to break down)
...
`/sensei:plan-phase {decimal_phase}`
...
- Don't use this for planned work at end of milestone (use /sensei:add-phase)
