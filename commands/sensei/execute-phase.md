...
@~/.claude/buildsensei/workflows/execute-phase.md
</execution_context>
...
4. **Execute waves**
   ...
   - Spawn `sensei-executor` for each plan in wave (parallel Task calls)
...
6. **Verify phase goal**
   - Spawn `sensei-verifier` subagent with phase directory and goal
   - Verifier checks must_haves against actual codebase (not SUMMARY claims)
   - Creates VERIFICATION.md with detailed report
   - Route by status:
     - `passed` → continue to step 7
     - `human_needed` → present items, get approval or feedback
     - `gaps_found` → present gaps, offer `/sensei:plan-phase {X} --gaps`
...
**Also available:**
- `/sensei:verify-work` — manual acceptance testing
- `/sensei:complete-milestone` — skip audit, archive directly
- `/sensei:add-phase <description>` — add another phase first
...
- `/sensei:verify-work {Z}` — manual testing before planning

---
```

After user runs `/sensei:plan-phase {Z} --gaps`:
1. Planner reads VERIFICATION.md gaps
2. Creates plans 04, 05, etc. to close gaps
3. User runs `/sensei:execute-phase {Z}` again
...
```
Task(prompt="Execute plan at {plan_01_path}\n\nPlan: @{plan_01_path}\nProject state: @.planning/STATE.md", subagent_type="sensei-executor")
...
See `@~/.claude/buildsensei/workflows/execute-phase.md` step `checkpoint_handling` for complete details.
</checkpoint_handling>
