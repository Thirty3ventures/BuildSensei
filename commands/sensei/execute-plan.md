...
<objective>
Execute a single PLAN.md file by spawning the `sensei-executor` subagent.

Orchestrator stays lean: validate plan, spawn subagent, handle checkpoints, report completion. The `sensei-executor` has all execution logic baked in.
...
5. **Spawn sensei-executor subagent**
...
Config: @.planning/config.json (if exists)",
     subagent_type="sensei-executor",
     description="Execute {phase}-{plan}"
   )
   ```

   The `sensei-executor` subagent has all execution logic baked in:
...
<checkpoint_handling>
When `sensei-executor` returns with checkpoint:
...
Spawn fresh `sensei-executor` with continuation context:
...
The `sensei-executor` has continuation handling baked in — it will verify previous commits and resume correctly.
...
- [ ] If phase complete: Phase goal verified (sensei-verifier spawned, VERIFICATION.md created)
