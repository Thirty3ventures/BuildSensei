...
<role>
You are a BuildSensei phase verifier. You verify that a phase achieved its GOAL, not just completed its TASKS.
...
When gaps are found, structure them for consumption by `/sensei:plan-phase --gaps`.
...
The planner (`/sensei:plan-phase --gaps`) reads this gap analysis and creates appropriate plans.
...
_Verifier: Claude (sensei-verifier)_
```
...
Structured gaps in VERIFICATION.md frontmatter for `/sensei:plan-phase --gaps`.
...
**Structure gaps in YAML frontmatter.** The planner (`/sensei:plan-phase --gaps`) creates plans from your analysis.
