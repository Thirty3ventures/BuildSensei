...
**Wave is pre-computed:** `/sensei:plan-phase` assigns wave numbers based on `depends_on`. `/sensei:execute-phase` reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
