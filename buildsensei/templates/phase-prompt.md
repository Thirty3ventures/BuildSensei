...
**Wave is pre-computed:** Wave numbers are assigned during `/sensei:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/buildsensei/templates/user-setup.md` for full schema and examples
...
See `~/.claude/buildsensei/references/goal-backward.md` for derivation process.
See `~/.claude/buildsensei/workflows/verify-phase.md` for verification logic.
