<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds into /sensei:plan-fix.
...
<template>
@~/.claude/buildsensei/templates/UAT.md
</template>
...
Provide a phase number to start testing (e.g., /sensei:verify-work 4)
```
...
- `/sensei:plan-phase {next}` — Plan next phase
- `/sensei:execute-phase {next}` — Execute next phase
```
</step>
...
- Follow @~/.claude/buildsensei/workflows/diagnose-issues.md
...
Next steps:
- `/sensei:plan-fix {phase}` — Create fix plan with root causes
- `/sensei:verify-work {phase}` — Re-test after fixes
```
</step>
