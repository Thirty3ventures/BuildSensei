...
1. ~/.claude/buildsensei/templates/phase-prompt.md
2. ~/.claude/buildsensei/references/plan-format.md
3. ~/.claude/buildsensei/references/scope-estimation.md
4. ~/.claude/buildsensei/references/checkpoints.md
5. ~/.claude/buildsensei/references/tdd.md
...
For niche domains (3D, games, audio, shaders, ML), suggest `/sensei:research-phase` before plan-phase.
</step>
...
- Any {phase}-RESEARCH.md (from /sensei:research-phase)
- Any DISCOVERY.md (from mandatory discovery)
- Any {phase}-CONTEXT.md (from /sensei:discuss-phase)
...
# Check for ecosystem research (from /sensei:research-phase)
cat .planning/phases/XX-name/${PHASE}-RESEARCH.md 2>/dev/null

# Check for phase context (from /sensei:discuss-phase)
...
**If neither exist:** Suggest /sensei:research-phase for niche domains, /sensei:discuss-phase for simpler domains, or proceed with roadmap only.
</step>
...
See `~/.claude/buildsensei/references/tdd.md` for TDD plan structure.
...
See ~/.claude/buildsensei/references/checkpoints.md for checkpoint structure.
...
See ~/.claude/buildsensei/references/scope-estimation.md for complete guidance.
</step>
...
Use template from `~/.claude/buildsensei/templates/phase-prompt.md`.
...
**Wave is pre-computed:** Wave numbers are assigned during planning (see `assign_waves` step). `/sensei:execute-phase` reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
```markdown
<execution_context>
@~/.claude/buildsensei/workflows/execute-plan.md
@~/.claude/buildsensei/templates/summary.md
@~/.claude/buildsensei/references/checkpoints.md
</execution_context>
```
...
`/sensei:execute-plan .planning/phases/XX-name/{phase}-01-PLAN.md`
...
[If 2+ plans: - `/sensei:execute-plan {phase}-01-PLAN.md` - run plans one at a time]
...
**TDD candidates get dedicated plans.** If "Create price calculator with discount rules" warrants TDD, create a TDD plan for it. See `~/.claude/buildsensei/references/tdd.md` for TDD plan structure.
</task_quality>
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/sensei:execute-phase {X}` next
</success_criteria>
