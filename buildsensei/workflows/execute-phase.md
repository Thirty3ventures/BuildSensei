...
**No dependency analysis needed.** Wave numbers are pre-computed during `/sensei:plan-phase`.
...
2. **Spawn all autonomous agents in wave simultaneously:**
   ...
   @~/.claude/buildsensei/references/tdd.md
   </execution_context>
...
| `gaps_found` | Present gap summary, offer `/sensei:plan-phase {phase} --gaps` |
...
- `/sensei:verify-work {X}` — manual testing before planning
```

User runs `/sensei:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) to close gaps
3. User then runs `/sensei:execute-phase {X}` again
...
1. Run `/sensei:execute-phase {phase}` again
