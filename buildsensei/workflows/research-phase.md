...
Triggered by /sensei:research-phase command when the domain is niche, complex, or Claude's training is likely stale.
...
Use /sensei:progress to see available phases.
```
Exit workflow.
...
**2. Phase context (if exists from /sensei:discuss-phase):**
```bash
...
**From ~/.claude/buildsensei/references/research-pitfalls.md:**
...
Use template from ~/.claude/buildsensei/templates/research.md
...
What's next?
1. Plan this phase (/sensei:plan-phase ${PHASE}) - RESEARCH.md will be loaded automatically
...
When /sensei:plan-phase runs after research:
