...
1. ~/.claude/buildsensei/templates/roadmap.md (milestone-grouped format)
...
- This contains context from `/sensei:discuss-milestone`
...
**If MILESTONE-CONTEXT.md exists (from /sensei:discuss-milestone):**
...
The main ROADMAP.md file holds full phase details for the active milestone. Archive files in `milestones/` are created only when a milestone ships (via `/sensei:complete-milestone`).
...
Plans:
- [ ] [N]-01: TBD (run /sensei:plan-phase [N] to break down)
...
This file was a handoff artifact from `/sensei:discuss-milestone`. Now that the milestone is created, the context is persisted in ROADMAP.md and the temporary file is no longer needed.
</step>
