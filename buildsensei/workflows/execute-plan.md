...
2. Use Task tool with subagent_type="sensei-executor":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="sensei-executor":
...
See ~/.claude/buildsensei/references/checkpoints.md for complete checkpoint guidance.
</step>
...
If you're reading this workflow because an orchestrator spawned you (vs running directly from /sensei:execute-plan), the orchestrator's prompt will include checkpoint return instructions. Follow those instructions when you hit a checkpoint.
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/buildsensei/templates/user-setup.md`.
...
Use ~/.claude/buildsensei/templates/summary.md for structure.
...
For commit message conventions, see ~/.claude/buildsensei/references/git-integration.md
</step>
...
`/sensei:execute-plan .planning/phases/{phase-dir}/{phase}-{next-plan}-PLAN.md`
...
**Also available:**
- `/sensei:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
**Also available:**
- `/sensei:verify-work {Z}` — manual acceptance testing before continuing
- `/sensei:discuss-phase {Z+1}` — gather context first
- `/sensei:research-phase {Z+1}` — investigate unknowns
...
**Also available:**
- `/sensei:verify-work` — manual acceptance testing before completing milestone
- `/sensei:add-phase <description>` — add another phase before completing
